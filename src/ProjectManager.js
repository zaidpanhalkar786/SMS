...
const PrimarySkillForm = ({ primarySkillsTable, setPrimarySkillsTable }) => {
  ...
  const fetchEmployeeSkills = async () => {
    try {
      ...
      const primarySkills = response.data;

    if (primarySkills && primarySkills.primarySkill) {
      const primarySkill = primarySkills.primarySkill;
      const mappedPrimarySkill = {
    ...
    <div>
      <div className="form-container">
        ...
        <Form.Group widths="equal">
          ...
          {!isEditMode ? (
            <Button style={{ marginLeft: '10px' }} color='purple' size='huge' onClick={handleSavePrimarySkill}>
              Save
            </Button>
          ) : (
            <Button style={{ marginLeft: '10px' }} color='purple' size='huge' onClick={handleUpdatePrimarySkill}>
              Update
            </Button>
          )}
          </Form.Group>
      ...
      <div className="table-container">
        ...
        {primarySkillsTable.length > 0 && (
          <Table celled border='5' className='skilltable'>
            <Table.Header>
              <Table.Row>
                <Table.HeaderCell width={5} style={{fontSize:'20px'}}>Skill ID</Table.HeaderCell>
                <Table.HeaderCell width={5}  style={{fontSize:'20px'}}>Years of Experience</Table.HeaderCell>
                <Table.HeaderCell width={5} style={{fontSize:'20px'}}>Certification</Table.HeaderCell>
                <Table.HeaderCell width={5} style={{fontSize:'20px'}}>Actions</Table.HeaderCell>
              </Table.Row>
            </Table.Header>
            ...
            <Table.Body>
              {primarySkillsTable.map((skill, index) => (
                <Table.Row key={index}>
                  <Table.Cell style={{fontSize:'20px'}}>{skill.skillId}</Table.Cell>
                  <Table.Cell style={{fontSize:'20px'}}>{skill.yearsOfExperience}</Table.Cell>
                  <Table.Cell style={{fontSize:'20px'}}>{skill.certification}</Table.Cell>
                  <Table.Cell>
                    <Icon
                      style={{fontSize:'20px'}}
                      name="edit"
...
const SecondarySkillForm = ({ secondarySkillsTable, setSecondarySkillsTable }) => {
  ...
  const handleEditSecondarySkill = (index) => {
    ...
    //console.log(selectedSecondarySkill._id);
    const skillId = selectedSecondarySkill._id ? selectedSecondarySkill._id : selectedSecondarySkill.skillId;
  ...
  const handleUpdateSecondarySkill = async() => {
    ...
    try {
      const token = localStorage.getItem('token')
      const email = localStorage.getItem('email')
      const response = await axios.put(`http://localhost:3001/secondaryskill/${secondarySkill._id}`, {
         email: email,
  ...
  const handleConfirmDeleteSecondarySkill = () => {
    const updatedTable = secondarySkillsTable.filter((_, i) => i !== selectedSecondarySkillIndex);
  ...
  const saveUpdatedTable = async (updatedTable) => {
    try {
      ...
      await axios.put('http://localhost:3001/secondaryskillupdatetable', {
        email: email,
    ...
    <div>
      <div className="form-container">
        ...
        <Form.Dropdown
         ...
         <Form.Group>
          {!isEditMode ? (
            <Button style={{ marginLeft: '10px' }} color='purple' size='huge' onClick={handleSaveSecondarySkill}>
              Save
            </Button>
          ) : (
            <Button style={{ marginLeft: '10px' }} color='purple' size='huge' onClick={handleUpdateSecondarySkill}>
              Update
            </Button>
          )}
          </Form.Group>
      ...
      <Modal style={{width:'450px', fontSize:'20px'}} open={deleteConfirmationOpen}>
        <Modal.Header>Confirm Delete</Modal.Header>
        <Modal.Content>
          <p>Are you sure you want to delete this secondary skill?</p>
        </Modal.Content>
        <Modal.Actions>
        <Button color='purple' size='large'  onClick={handleConfirmDeleteSecondarySkill}>
            Yes
          </Button>
  ...
  const ProjectManager = () => {
   ...
   // const [searchResults, setSearchResults] = useState([]);
    ...
    const panes = [
        ...
        {
          menuItem: { content: <span style={{ fontSize: '2.2rem', color: 'purple' }}>Secondary Skills</span> },
          render: () => (
            <Tab.Pane>
              <Divider hidden style={{margin:'7px'}} />
              <SecondarySkillForm
